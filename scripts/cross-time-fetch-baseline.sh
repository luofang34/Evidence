...
#
# Extracted from `.github/workflows/ci.yml` so its fail-closed behavior
# is testable with a fake `gh` (see the `cross_time_fetch_baseline`
# integration test) instead of only asserting the workflow text exists.
...
if ! gh run download "$last_run" --name xhost-Linux --dir "$dest" 2>/dev/null; then
  fail_closed "Baseline artifact xhost-Linux for base SHA ${BASE_SHA} (run ${last_run}) is unavailable (expired or missing)."
fi

if [ ! -f "$dest/deterministic-manifest.json" ]; then
...
jq '{rustc, cargo, llvm_version, cargo_lock_hash, rustflags}' "$dest/deterministic-manifest.json"
