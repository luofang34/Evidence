...
use evidence::bundle::TestOutcome;
use evidence::diagnostic::FixHint;
...
fn synth_trace(
    ...
    // the test.
    let trace = evidence::trace::read_all_trace_files(root.to_str().unwrap()).expect("read trace");
    std::mem::forget(td);
    trace
}
