...
pub enum CorpusError {
    ...
    /// The index lists entries for a node kind this tool version does
    /// not load yet; refusing beats silently ignoring indexed data.
    #[error("corpus index lists {kind} entries, which this tool version cannot load")]
    UnimplementedKind {
        /// Index key of the unloadable kind (e.g. `sources`).
        kind: &'static str,
    },
