...
mod unix {
    ...
    fn runner_success_and_typed_failures() {
        ...
        let (_dir, exe, input) = fake("#!/bin/sh\nkill -TERM $$\n");
        let error = run_pdftotext_blocking(&exe, &input, &test_bounds())
            .expect_err("signal termination fails");
    ...
    fn runner_bounds_timeout_and_output_limits() {
        ...
        let error =
            run_pdftotext_blocking(&exe, &input, &bounds).expect_err("oversized output fails");
        assert!(matches!(
            error,
        ...
        let (_dir, exe, input) = fake(script);
        let error =
            run_pdftotext_blocking(&exe, &input, &test_bounds()).expect_err("extra file fails");
        assert!(matches!(
            error,
            PdfRunError::OversizedOutput {
                what: "file-count",
