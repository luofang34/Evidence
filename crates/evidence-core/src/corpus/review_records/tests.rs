...
use crate::corpus::{
    ...
    ReviewNode, ReviewTarget, ReviewTargetKind,
};
...
fn approve_and_reject_records_round_trip() {
    ...
    assert_eq!(approval.target, ReviewTarget::Requirement(REQ_A.to_string()));
...
fn approve_typed<'a>(uid: &'a str, id: &'a str, kind: &'a str, target_uid: &'a str) -> TypedSpec<'a> {
    TypedSpec {
        uid,
        id,
        kind,
...
fn typed_target_records_round_trip_and_mixed_layouts_load() {
    ...
    for field in [
        legacy_review.content_schema,
        typed_review.content_schema,
    ] {
        assert_eq!(field, 1);
    }
    assert_eq!(
    ...
    let split = load_corpus(&[
        ("x/legacy", legacy.as_str()),
        ("y/typed", typed.as_str()),
    ])
    .expect("split layout loads");
    ...
    let patch_file = typed_review_file(&[approve_typed(REV_3, "REV-003", "curated_patch", PATCH_A)]);
    let dir = tempfile::tempdir().unwrap();
...
fn target_shape_and_kind_violations_fail_closed() {
    ...
    let unknown_target_field =
        valid_typed.replace("uid = \"req_", "extra = true, uid = \"req_");
    let cross_kind_req = valid_typed.replace(REQ_A, PATCH_A);
    let cross_kind_patch = typed_review_file(&[approve_typed(
        REV_1, "REV-001", "curated_patch", REQ_A,
    )]);
    ...
    let legacy_missing_target = review_file(&[approve(REV_1, "REV-001")])
        .replace("requirement_uid = \"req_00000000-0000-4000-8000-00000000000a\"\n", "");
    ...
    let cases: [Case; 9] = [
        ...
        ("curated-patch kind with requirement uid", cross_kind_patch, |err| {
            matches!(
                err,
                ReviewError::NativeUidPrefix {
                    expected: "patch_",
        ...
        ("typed target in a schema-1 record", legacy_with_target, |err| {
            matches!(
                err,
        ...
        ("schema-1 record without requirement_uid", legacy_missing_target, |err| {
            matches!(
                err,
                ReviewError::ReviewTargetShape {
                    schema_version: 1,
        ...
        ("legacy field in a schema-2 record", typed_with_legacy_field, |err| {
            matches!(
                err,
                ReviewError::ReviewTargetShape {
                    schema_version: 2,
        ...
        ("schema-2 record without target", typed_missing_target, |err| {
            matches!(
                err,
                ReviewError::ReviewTargetShape {
                    schema_version: 2,
