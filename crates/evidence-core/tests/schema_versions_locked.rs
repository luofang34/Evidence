//! Grep-regression for `evidence_core::schema_versions`.
//!
...
//!   matching constant in `evidence_core::schema_versions`;
...
fn workspace_root() -> PathBuf {
    // This test lives in `crates/evidence-core/tests/`.
...
fn no_version_literals_outside_schema_versions() {
    ...
    if !hits.is_empty() {
        let mut msg = String::from(
            "Found version-string literals outside evidence_core::schema_versions. \
...
fn schema_constants_pinned_at_001() {
    let path = workspace_root().join("crates/evidence-core/src/schema_versions.rs");
    let text =
