...
use evidence_core::bundle::{TestOutcome, parse_cargo_test_output_with_outcomes};
...
fn outcome_map_keys_include_binary_prefix() {
    ...
    assert_eq!(
        outcomes
            .get("evidence_core::diagnostic::tests::severity_rejects_unknown_variant")
            .copied(),
        Some(TestOutcome::Passed),
    );
    assert_eq!(
        outcomes
            .get("evidence_core::trace::entries::tests::test_hlr_entry_fields")
            .copied(),
...
fn parser_contract_requires_colorless_input() {
    ...
    assert_eq!(
        outcomes
            .get("evidence_core::diagnostic::tests::works")
            .copied(),
        Some(TestOutcome::Passed),
    );
}
