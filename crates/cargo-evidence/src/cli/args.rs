...
pub enum Commands {
    ...
    Generate {
        ...
        /// Opt into online dependency resolution (development mode
        /// only). Without this flag every cargo subprocess in the
        /// pipeline runs `--locked --offline`, resolving only from
        /// the committed lockfile and the local cargo cache. The
        /// opt-in is recorded in the bundle (`index.json`
        /// `resolution_policy`) and cannot produce a cert/record
        /// claim: cert/record profiles refuse the flag with
...
pub use choices::{CheckMode, CoverageChoice, OutputFormat};
